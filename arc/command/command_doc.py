...
class Section:
    ...
    def __init__(self, header: str, content: t.Any):
        self.header = header
    ...
    def __repr__(self):
        return f"Section(header='{self.header}', content={self.content!r})"

    def format_header(self) -> str:
        return colorize(self.header.upper(), effects.BOLD)

    def format_content(self) -> str:
        return textwrap.indent(str(self.content).strip("\n"), TAB)


class ArgumentSection(Section):
    content: list[Param]

    def format_content(self) -> str:

        data = [
            (format(param, "arguments"), param.description) for param in self.content
        ]

        longest = len(max(data, key=lambda v: len(v[0]))[0])

        string = "\n".join(
            f"{formatted:<{longest}}  {desc}" for formatted, desc in data
        )

        return textwrap.indent(string, TAB).strip("\n")
...
class CommandDoc:
    ...
    def __init__(self, state: ExecutionState):
        self.state = state
        self.command = self.state.command
        self.schema = self.command.schema()
        self.sections: dict[str, Section] = {}

        if self.state.command.doc:
            self.doc = self.parse_docstring(self.state.command.doc)
        else:
            self.doc = {}
        ...
        for header, content in self.doc.items():
            if header != "description":
                self.add_section(header, content)
    ...
    def add_section(self, header: str, content: str):
        self.sections[header] = Section(header, content)

    def update_section(self, header: str, content: str):
        if header in self.sections:
            section = self.sections[header]
            section.content += content
        else:
            self.add_section(header, content)

    def add_usage(self):
        if self.command.is_namespace():
            command_str = f"{self.command.name}{config.namespace_sep}<subcommand>"
            args_str = "[arguments ...]"
        elif self.state.root == self.command:
            command_str = "<command>"
            args_str = "[arguments ...]"
        else:
            command_str = config.namespace_sep.join(self.state.command_namespace)
            args_str = self.add_args_str()

        self.add_section(
            "usage",
            f"{colorize(self.state.root.name, fg.ARC_BLUE)} "
            f"{colorize(command_str, effects.UNDERLINE)} {args_str}",
        )

    def add_args_str(self):
        args = []
        for arg in (
            arg
            for arg in self.command.executable.params.values()
        ...
        for arg in self.command.executable.pos_params.values():
            args.append(format(arg, "usage"))

        return " ".join(args)

    def add_description(self):
        if self.command.description:
            self.add_section("description", self.command.description)
        elif "description" in self.doc:
            self.add_section("description", self.doc["description"])

    def add_arguments(self):
        if "arguments" in self.doc:
            self.add_section("arguments", self.doc["arguments"])
        else:
            self.sections["arguments"] = ArgumentSection(
                "arguments", self.command.executable.visible_params.values()
            )
    ...
    def add_other_sections(self):
        ...

    @classmethod
    def parse_docstring(cls, doc: str):
        ...
        """
        parsed: dict[str, str] = {cls.DEFAULT_SECTION: ""}
        lines = [line.strip() for line in doc.split("\n")]

        current_section = cls.DEFAULT_SECTION

        for line in lines:
            if line.startswith("#"):
                current_section = line[1:].strip().lower()
                parsed[current_section] = ""
            elif line != "" or current_section != cls.DEFAULT_SECTION:
